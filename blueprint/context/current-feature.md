...
- [ ] **Step 1 – `telegram_threads` + Repository** – Modell, Alembic-Migration
...
- [ ] **Step 2 – `createForumTopic` im Notifier** – `create_topic(name, icon_color)`
...
- [ ] **Step 3 – Pipeline: ein Topic pro Match** – im `_notify`-Zweig zuerst
...
- [ ] **Step 4 – Konfiguration und Doku** – `.env.example` (Supergruppen-ID
...
- **Reihenfolge im Notify-Zweig:** Topic erzeugen → senden → Zuordnung und
  `notified_at` → Commit. Nie `notified_at` setzen, bevor die Zuordnung steht.
